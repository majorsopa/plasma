...
class GlobalContext:
    ...
    def parse_args(self):
        ...
        parser.add_argument(
            "-g", "--graph", action="store_true", help="Generate a file graph.dot."
        )
        parser.add_argument(
            "--noandif",
            action="store_true",
        ...
        parser.add_argument(
            "--vim", action="store_true", help="Generate syntax colors for vim"
        )
        parser.add_argument(
            "-s", "--symbols", action="store_true", help="Print all symbols"
        )
        parser.add_argument(
            "--sections", action="store_true", help="Print all sections"
        )
        parser.add_argument(
            "--dump", action="store_true", help="Dump asm without decompilation"
        )
        parser.add_argument(
            "-l",
            "--lines",
        ...
        parser.add_argument(
            "--nbytes", type=int, default=0, metavar="N", help="Print n bytes."
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--rawbe", action="store_true", help="If not set it's in little endian"
        )
        parser.add_argument(
            "-na",
            "--noautoanalyzer",
        ...
        parser.add_argument(
            "--db_path",
            default=None,
            help="Database path ('.<exe_name>.db' by default).",
        )
    ...
    def load_file(self, filename=None):
        ...
        try:
            dis = Disassembler(
                filename, self.raw_type, self.raw_base, self.raw_big_endian, self.db
            )
        except ExcArch as e:
...
class AddrContext:
    ...
    def init_address(self, entry, quiet=False):
        ...
        if entry is None:
            ...
            self.entry = (
                self.gctx.db.symbols.get("main", None)
                or self.gctx.db.symbols.get("_main", None)
                or self.gctx.dis.binary.get_entry_point()
            )
        ...
        self.entry = (
            self.gctx.db.demangled.get(entry, None)
            or self.gctx.db.symbols.get(entry, None)
            or self.gctx.dis.binary.section_names.get(entry, None)
        )
        ...
        try:
            ...
            if not correctly_ended:
                ...
                ast.nodes.insert(
                    0,
                    Ast_Comment(
                        "WARNING: maybe there is a bug, a second pass was necessary"
                    ),
                )
                ast.nodes.insert(0, Ast_Comment(""))
                ast.nodes.insert(0, Ast_Comment(""))
